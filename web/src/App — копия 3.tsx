...
export default function App() {
  ...
  const eN = selected?.electric_expected ?? 3;
  ...
  const [infoElectricExpected, setInfoElectricExpected] = useState("3");
  ...
  async function openInfo(apartmentId: number) {
    ...
    try {
      ...
      setInfoElectricExpected(String((data.apartment as any)?.electric_expected ?? 3));
    ...
    try {
      ...
      await fetch(`/api/admin/ui/apartments/${apartmentId}`, {
        ...
        body: JSON.stringify({
          ...
          electric_expected: Number(infoElectricExpected),
  ...
  async function loadApartments(selectIfEmpty = true) {
    ...
    const items = (data.items ?? []).map((x) => ({
      ...
      electric_expected: x.electric_expected ?? 3,
    }));
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse" }}>
                    <thead>
                      <tr>
                        ...
                        {eN >= 1 && (
                          <th style={{ textAlign: "left", padding: 8, borderBottom: "1px solid #eee" }}>T1</th>
                        )}
                        {eN >= 2 && (
                          <th style={{ textAlign: "left", padding: 8, borderBottom: "1px solid #eee" }}>T2</th>
                        )}
                        {eN >= 3 && (
            ...
            {infoLoading ? (
              ...
              <div style={{ marginTop: 12, display: "grid", gap: 12 }}>
                ...
                <div style={{ borderTop: "1px solid #eee", paddingTop: 12 }}>
                  <div style={{ fontWeight: 900, marginBottom: 8 }}>Параметры квартиры</div>

                  <label style={{ display: "grid", gap: 6, marginBottom: 12 }}>
                    <div style={{ fontWeight: 800 }}>Сколько фото электро ждём</div>
                    <select
                    ...
                    <div style={{ color: "#666", fontSize: 12 }}>
                      Это влияет на интерфейс: сколько колонок T1/T2/T3 показывать в таблице месяцев.
                    </div>
                  </label>
