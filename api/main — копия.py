...
def _calc_month_bill(conn, apartment_id: int, ym: str) -> Dict[str, Any]:
    """
    Возвращает:
      - is_complete_photos: есть ли все текущие показания, нужные для расчета (cold/hot/electric 1..3)
      - total_rub: сумма ₽, если можно посчитать (есть прошлый месяц + тарифы)
      - missing: что не хватает
    """
    cur_cold = _get_reading(conn, apartment_id, ym, "cold", 1)
    cur_hot = _get_reading(conn, apartment_id, ym, "hot", 1)
    cur_e1 = _get_reading(conn, apartment_id, ym, "electric", 1)
    cur_e2 = _get_reading(conn, apartment_id, ym, "electric", 2)
    cur_e3 = _get_reading(conn, apartment_id, ym, "electric", 3)
    ...
    if cur_hot is None:
        missing.append("hot")
    if cur_e1 is None:
        missing.append("electric_t1")
    if cur_e2 is None:
        missing.append("electric_t2")
    if cur_e3 is None:
        missing.append("electric_t3")

    is_complete_photos = (len(missing) == 0)

    tariff = _get_tariff_for_month(conn, ym)
    if not tariff:
        return {
            "ym": ym,
            "is_complete_photos": is_complete_photos,
            "missing": missing,
            "total_rub": None,
            "reason": "no_tariff",
        }
    ...
    prev_cold = _get_reading(conn, apartment_id, prev, "cold", 1)
    prev_hot = _get_reading(conn, apartment_id, prev, "hot", 1)
    prev_e1 = _get_reading(conn, apartment_id, prev, "electric", 1)
    prev_e2 = _get_reading(conn, apartment_id, prev, "electric", 2)
    prev_e3 = _get_reading(conn, apartment_id, prev, "electric", 3)
    ...
    d_cold = delta(cur_cold, prev_cold)
    d_hot = delta(cur_hot, prev_hot)
    d_e1 = delta(cur_e1, prev_e1)
    d_e2 = delta(cur_e2, prev_e2)
    d_e3 = delta(cur_e3, prev_e3)

    d_sewer = None
    if d_cold is not None and d_hot is not None:
        d_sewer = d_cold + d_hot
    ...
    r_cold = rub(d_cold, tariff["cold"])
    r_hot = rub(d_hot, tariff["hot"])
    r_e1 = rub(d_e1, tariff["electric_t1"])
    r_e2 = rub(d_e2, tariff["electric_t2"])
    r_e3 = rub(d_e3, tariff["electric_t3"])
    r_sewer = rub(d_sewer, tariff["sewer"])

    components = [r_cold, r_hot, r_e1, r_e2, r_e3, r_sewer]
    if any(x is None for x in components):
        return {
            ...
            "missing": missing,
            "total_rub": None,
            "reason": "need_prev_month_or_missing_values",
            "tariff_month_from": tariff["month_from"],
        }

    total_rub = float(sum(components))
    return {
        "ym": ym,
        "is_complete_photos": is_complete_photos,
        "missing": missing,
...
def _assign_and_write_electric_sorted(apartment_id: int, ym: str, new_value: float) -> int:
    """
    Электро может приходить 1..3 фото.
    Правило: значения сортируем по возрастанию и кладём в meter_index 1..N.
    Если в месяце уже есть MANUAL записи по electric — не переставляем их местами.
    Возвращает meter_index, который присвоен ИМЕННО этому new_value.
    """
    if not db_ready():
        return 1
    ...
    with engine.begin() as conn:
        ...
        existing = [{"idx": int(r[0]), "val": float(r[1]), "source": str(r[2])} for r in rows]
        has_manual = any(x["source"] == "manual" for x in existing)

        for x in existing:
            if abs(x["val"] - float(new_value)) <= tol:
                return int(x["idx"])

        if has_manual:
            used = {x["idx"] for x in existing}
            free = [i for i in (1, 2, 3) if i not in used]
            assigned = free[0] if free else 3
            ...
            conn.execute(
                text("""
                    ...
                    VALUES
                        (:aid, :ym, 'electric', :idx, :val, 'ocr', :val)
                    ...
                    DO UPDATE SET
                        ...
                        updated_at = now()
                """),
                {"aid": int(apartment_id), "ym": ym, "idx": int(assigned), "val": float(new_value)},
            )
            return int(assigned)

        values = [x["val"] for x in existing] + [float(new_value)]
            ...
            conn.execute(
                text("""
                    ...
                    VALUES
                        (:aid, :ym, 'electric', :idx, :val, 'ocr', :val)
                """),
                {"aid": int(apartment_id), "ym": ym, "idx": int(i), "val": float(v)},
            )
            if abs(float(v) - float(new_value)) <= tol:
                assigned = int(i)
...
class StatusPatch(BaseModel):
    rent_paid: Optional[bool] = None
    meters_paid: Optional[bool] = None
    meters_photo_cold: Optional[bool] = None
    meters_photo_hot: Optional[bool] = None
    meters_photo_electric: Optional[bool] = None


class TariffIn(BaseModel):
    month_from: str = Field(..., alias="ym_from")
    cold: float
    hot: float
    sewer: float
...
# -----------------------
# Dashboard: patch statuses (лампочки)
# -----------------------
...
def upsert_tariff(payload: TariffIn):
    ...
    base_electric = payload.electric
    if base_electric is None:
        base_electric = payload.electric_t1 or payload.electric_t2 or payload.electric_t3 or 0.0

    e1 = payload.electric_t1 if payload.electric_t1 is not None else base_electric
    e2 = payload.electric_t2 if payload.electric_t2 is not None else base_electric
    e3 = payload.electric_t3 if payload.electric_t3 is not None else base_electric

    with engine.begin() as conn:
        conn.execute(
            text("""
                INSERT INTO tariffs
                    (month_from, cold, hot, electric, electric_t1, electric_t2, electric_t3, sewer, created_at, updated_at)
                VALUES
                    (:month_from, :cold, :hot, :electric, :electric_t1, :electric_t2, :electric_t3, :sewer, now(), now())
                ON CONFLICT (month_from)
                DO UPDATE SET
                    cold = EXCLUDED.cold,
                    hot = EXCLUDED.hot,
            ...
            """),
            {
                ...
                "cold": payload.cold,
                "hot": payload.hot,
                "electric": float(base_electric),
                "electric_t1": float(e1),
                "electric_t2": float(e2),
                "electric_t3": float(e3),
                "sewer": payload.sewer,
            },
        )
